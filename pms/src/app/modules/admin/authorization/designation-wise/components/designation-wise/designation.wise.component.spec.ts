...
import { DesignationWiseComponent } from './designation.wise.component';
import { DesignationWiseService } from '../../services/designationwise.service';
import { PermissionService } from '../../../../../../common/services/permission.service';
import { CoreModule } from '../../../../../../common/common.module';
import { AppRoutingModule } from '../../../../../../app-routing.module';
import { PermissionFormControlName, designationOptions, moduleOptions, responseData, responseViewAddData, singleResponseData, singleResponseFalseData } from '../../data/testdata';
import { PageAccessTypes, Pages } from '../../../../../../common/constants/Enums';

describe('DesignationWiseComponent', () => {
    let component: DesignationWiseComponent;
    let fixture: ComponentFixture<DesignationWiseComponent>;
    let service: DesignationWiseService;
    ...
    beforeEach(async () => {
        ...
        await TestBed.configureTestingModule({
            ...
            declarations: [DesignationWiseComponent],
            providers: [
        ...
        fixture = TestBed.createComponent(DesignationWiseComponent);
        globalService = TestBed.inject(GlobalService) as jasmine.SpyObj<GlobalService>;
        service = TestBed.inject(DesignationWiseService);
        permissionService = TestBed.inject(PermissionService) as jasmine.SpyObj<PermissionService>;
        component = fixture.componentInstance;
        component.isAddPermission = permissionService.checkAccessPermission(Pages.Designation, PageAccessTypes.Add);
        component.isEditPermission = permissionService.checkAccessPermission(Pages.Designation, PageAccessTypes.Edit);
    ...
    it('should set designation list when getDesignations succeeds', () => {
        const mockBookData = {
            isSuccess: true,
            data: designationOptions
        };
        spyOn(service, 'getDesignations').and.returnValue(of(mockBookData));
        component.ngOnInit();
        expect(service.getDesignations).toHaveBeenCalled();
    });

    it('should set designation list when getModules succeeds', () => {
        const mockBookData = {
            isSuccess: true,
            data: moduleOptions
        };
        spyOn(service, 'getModules').and.returnValue(of(mockBookData));
        component.ngOnInit();
        expect(service.getModules).toHaveBeenCalled();
    });

    it('should call onEnterPress in designation enter', () => {
        const consoleSpy = spyOn(component, 'getPageRights');
        if (component.designationDropdownConfig.onEnterPress) {
            component.designationDropdownConfig.onEnterPress();
        }
    ...
    it('should update specific permission when view checkbox is changed to false', () => {
        ...
        expect(component.headerForm.get(`${PermissionFormControlName.VIEW}${id}`)?.value).toBe(false);
        expect(component.headerForm.get(`${PermissionFormControlName.ADD}${id}`)?.value).toBe(false);
        expect(component.headerForm.get(`${PermissionFormControlName.EDIT}${id}`)?.value).toBe(false);
        expect(component.headerForm.get(`${PermissionFormControlName.DELETE}${id}`)?.value).toBe(false);
        expect(component.headerForm.get(`${PermissionFormControlName.EXPORT}${id}`)?.value).toBe(false);
        expect(component.headerForm.get(`${PermissionFormControlName.ALL}${id}`)?.value).toBe(false);
        expect(component.headerForm.get('canViewHeader')?.value).toBe(false);
        expect(component.headerForm.get('canAddHeader')?.value).toBe(false);
        expect(component.headerForm.get('canEditHeader')?.value).toBe(false);
    ...
    it('should reset page rights properly', () => {
        ...
        component.designationOptions = designationOptions;
        ...
        component.filterForm = formBuilder.group({
            designation: designationOptions[1].id,
        ...
        expect(component.filterForm.get('designation')?.value).toBe(designationOptions[0].id);
